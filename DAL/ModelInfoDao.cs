...
namespace DAL
{
    public class ModelInfoDao
    {
        ...
        public ModelFile queryModelInfoNo(string fileNo)
        {
            ModelFile modelFile = null;
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT uuid,file_no,fileName,fileDescription,op_user,create_time FROM t_model_file where file_no=@fileNo AND del_flag is null ");
            MySqlParameter[] parameters = {
                new MySqlParameter("@fileNo", MySqlDbType.VarChar, 900),
            };
            parameters[0].Value = fileNo;
        ...
        public List<ModelFile> queryModelFileList()
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT uuid,file_no,fileName,fileDescription,op_user,create_time FROM t_model_file WHERE del_flag is null ");
            MySqlParameter[] parameters = {
